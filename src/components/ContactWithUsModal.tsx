...
const countries = [
    {
        code: '+44',
        name: 'United Kingdom',
        flag: <img src="https://flagcdn.com/24x18/gb.png" alt="UK" width={24} height={18} style={{ borderRadius: '4px' }} />
    },
    {
        code: '+1',
        name: 'United States',
        flag: <img src="https://flagcdn.com/24x18/us.png" alt="US" width={24} height={18} style={{ borderRadius: '4px' }} />
    },
    {
        code: '+92',
        name: 'Pakistan',
        flag: <img src="https://flagcdn.com/24x18/pk.png" alt="Pakistan" width={24} height={18} style={{ borderRadius: '4px' }} />
    }
];

const ContactWithUsModal: React.FC<ContactModalProps> = ({ isOpen, onClose }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const fullPhone = formData.phone ? `${countries[countryIndex].code} ${formData.phone}` : '';
    ...
    return (
        ...
        >
            <div
                className="bg-white rounded-[24px] max-w-[559px] py-6 sm:py-8 px-5 md:px-[50px] w-full max-h-[90vh] overflow-y-auto hide-scrollbar transform transition-transform duration-300 scale-100"
                onClick={e => e.stopPropagation()}
            >
                {/* Header */}
                <div className="w-full flex items-end justify-end mb-2 sm:mb-[28px]">
                    <button onClick={onClose} className="cursor-pointer" type="button" aria-label="Close modal">
                ...
                {/* Title */}
                <h2 className="text-[24px] sm:text-[32px] leading-[26px] sm:leading-[36px] font-semibold tracking-[-2%] text-[#3A96AF] mb-4 sm:mb-6">
                            ...
                            <div className="relative" ref={dropdownRef}>
                                ...
                                >
                                    <div className="flex items-center gap-1">
                                        ...
                                        <span className="text-[16px] leading-[18px] text-[#252525] font-normal tracking-[0.5px]">{countries[countryIndex].code}</span>
                                    </div>
                                    <svg
                                        viewBox="0 0 24 24"
                                        fill="none"
                                        xmlns="http://www.w3.org/2000/svg"
                                        className={`transition-transform duration-200 w-5 h-5 ${dropdownOpen ? 'rotate-180' : ''}`}
                                    >
                                        <path
                                    ...
                                    <div className="absolute left-0 mt-2 w-[180px] bg-white border border-gray-200 shadow-lg z-10">
                                        {countries.map((country, idx) => (
                                            <button
                                                key={country.code}
                                                type="button"
                                                className={`flex items-center gap-2 w-full px-3 py-2 hover:bg-gray-100 ${countryIndex === idx ? 'bg-[#F8FAFC]' : ''}`}
                                                onClick={() => {
                                                    setCountryIndex(idx);
                                                    setDropdownOpen(false);
                                                }}
