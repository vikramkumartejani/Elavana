...
const SignUp = () => {
    ...
    return (
        <div className="w-full max-w-[597px] mx-auto h-auto bg-white px-5 py-8 sm:p-10 flex flex-col items-center justify-center" style={{ boxShadow: '0px 4px 24px 0px #25252514' }}>
            <h2 className="text-[24px] sm:text-[28px] leading-[30px] sm:leading-[38px] tracking-[0.5px] font-bold text-center mb-2 sm:mb-4 text-[#252525]">Create your Account</h2>
            <p className="text-[#676D75] text-[16px] sm:text-[18px] sm:leading-[22px] tracking-[0.5px] font-light text-center mb-8">Start your journey by signing up below.</p>
            <form className="w-full flex flex-col gap-4">
                <div className="flex sm:flex-row flex-col gap-4 sm:gap-6">
                    <div className="flex flex-col sm:w-1/2 gap-2 sm:gap-3">
                        <label className="text-[14px] sm:text-[16px] leading-[14px] sm:leading-[20px] font-medium text-black tracking-[0.5px]">First Name</label>
                        <input type="text" placeholder="Jhon" className={getInputClassName()} />
                    </div>
                    <div className="flex flex-col sm:w-1/2 gap-2 sm:gap-3">
                        <label className="text-[14px] sm:text-[16px] leading-[14px] sm:leading-[20px] font-medium text-black tracking-[0.5px]">Last Name</label>
                        <input type="text" placeholder="Doe" className={getInputClassName()} />
                    </div>
                </div>

                <div className="flex flex-col gap-2 sm:gap-3">
                    <label className="text-[14px] sm:text-[16px] leading-[14px] sm:leading-[20px] font-medium text-black tracking-[0.5px]">Email</label>
                ...
                <div className="flex flex-col gap-2 sm:gap-3 relative">
                    <label className="text-[14px] sm:text-[16px] leading-[14px] sm:leading-[20px] font-medium text-black tracking-[0.5px]">Password</label>
                ...
                <div className="flex flex-col gap-2 sm:gap-3">
                    <label className="text-[14px] sm:text-[16px] leading-[14px] sm:leading-[20px] font-medium text-black tracking-[0.5px]">Phone Number</label>
                    <PhoneNumberDropdown />
                </div>

                <div className="flex items-center gap-2 sm:gap-3">
                    <Checkbox id="terms" />
                    <label htmlFor="terms" className="text-[14px] leading-[18px] tracking-[0.5px] text-[#676D75] font-normal select-none cursor-pointer">I accept the terms & condition</label>
                </div>

                <RippleButton type="submit" className="mt-2 sm:mt-4 w-full bg-[#3A96AF] hover:bg-[#2a7a8f] text-white font-semibold h-[44px] sm:h-[56px] rounded-lg sm:rounded-[12px] transition duration-200 text-[16px] sm:text-[20px] sm:leading-[24px] tracking-[0.5px]">Sign Up</RippleButton>
            </form>

            <div className="mt-6 sm:mt-8 text-center text-[14px] sm:text-[20px] sm:leading-[24px] font-light tracking-[0.5px] text-black">
                Have an account?{' '}
                <Link href='/login' className="text-[#3996B5] hover:underline font-semibold">Log In</Link>
            </div>
        </div>
    );
};
