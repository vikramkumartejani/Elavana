...
const Login: React.FC = () => {
  ...
  const getInputClassName = (hasError: boolean): string => {
    return `w-full rounded-lg sm:rounded-[12px] bg-[#F8FAFC] outline-none px-4 sm:px-5 py-3.5 sm:py-[18px] placeholder:text-[#9A9EA6] text-[14px] sm:text-[16px] leading-[18px] sm:leading-[20px] tracking-[0.5px] transition-colors ${
      hasError ? "" : "border border-transparent focus:border-[#3A96AF]"
    }`;
  };
  ...
  return (
    ...
    >
      <h2 className="text-[#252525] text-[24px] sm:text-[40px] sm:leading-[44px] font-semibold text-center mb-2 sm:mb-4">
        Welcome Back
      </h2>
      <p className="text-[#676D75] text-[16px] sm:text-[24px] sm:leading-[28px] text-center mb-8">
      ...
      <form onSubmit={handleSubmit} className="w-full flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-2 sm:gap-3">
          <label
            htmlFor="email"
            className="text-[14px] sm:text-[20px] leading-[14px] sm:leading-[16px] font-medium text-black tracking-[0.5px]"
          >
            Email
          </label>
          <input
        ...
        <RippleButton
          ...
          className={`sm:mt-4 border border-[#9CB0C9] rounded-lg sm:rounded-[12px] h-[44px] sm:h-[48px] text-[#FFFFFF] text-[16px] sm:text-[20px] sm:leading-[24px] font-semibold tracking-[0.5px] transition duration-200 ${
            isSubmitting
