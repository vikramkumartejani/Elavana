...
const ServicesHeader: React.FC<ServicesHeaderProps> = ({ sortBy, setSortBy, title = 'Available Services' }) => {
    ...
    return (
        <div className="flex sm:items-center justify-between w-full mb-4 md:mb-[31px] sm:flex-row flex-col gap-2">
            <h2 className="text-[18px] md:text-[24px] md:leading-[30px] font-medium text-[#252525]">
                {title}
            </h2>
            <div className="flex items-center space-x-4">
                <div className="relative sm:w-fit w-full">
                    ...
                    >
                        <span className="text-[14px] leading-[20px] font-medium text-[#252525]">{getSortDisplayName(sortBy as SortOption)}</span>
                        ...
                        <div className="absolute top-full right-0 mt-1 bg-white border border-[#E8ECF4] rounded-lg shadow-lg z-50 w-full sm:min-w-[180px]">
                            {sortOptions.map(option => (
                                <button
                                    key={option.id}
                                    onClick={() => {
                                        setSortBy && setSortBy(option.id as string);
